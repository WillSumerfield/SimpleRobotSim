...
class Agent():
    ...
    def train(self, total_timesteps=1e8):
        rew_callback = StopTrainingOnRewardThreshold(reward_threshold=200, verbose=0)
        ...
        checkpoint_callback = CheckpointCallback(save_freq=1e5, save_path="./checkpoints", name_prefix="ppo_grasper")
        model = PPO("MlpPolicy", self.envs, verbose=1, device="cpu")
